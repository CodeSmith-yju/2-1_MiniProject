document.addEventListener('DOMContentLoaded', function() {  
    ...
    // PC게임 메뉴와 모바일게임 메뉴 버튼, 언어 버튼, 모든메뉴 버튼, 로그아웃 드롭다운 버튼 이외에 누르면 드롭다운 메뉴가 닫히게 하는 코드
    ...
    // 필터를 이용하여 현재 토글 된 드롭다운 메뉴가 아니라면 다른 드롭다운 메뉴에 클래스를 삭제 하는 기능
    ...
    // 인기게임 버튼을 누르면 클래스 이름이 새로 생성되고 나머지 버튼에 있던 클래스는 삭제하는 것
    ...
    // 자동으로 인덱스 값을 변경하는 함수
    // 가지고 있는 인덱스 length와 나머지를 구해서 무한 루프를 구현 (length와 같아지면 인덱스 0으로 변경됨)
    ...
    // 공지사항 자동으로 바뀌는 기능
    let notice_tit_list = ["4/18(화) 카카오페이 결제수단으로 캐시 충전 불가 현상 안내 (정상화)", "5/1(월) 근로자의 날 고객센터 휴무 안내", 
                            "5/29(월) 부천님 오신날 대체 공휴일 관련 고객센터 휴무 안내"];
    ...
    // 해당 인덱스의 href 속성 값과 텍스트 내용을 배열 안의 값으로 변경하게 바꿈
